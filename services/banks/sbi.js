const { fetchBankJson, normalizeBankPayload } = require("./http");
const { FALLBACK_BANK_RATES } = require("./fallbackRates");

async function fetchSbiRates() {
  const fallback = FALLBACK_BANK_RATES.sbi;
  const url = process.env.SBI_FD_API_URL || process.env.SBI_API_URL;
  const apiKey = process.env.SBI_API_KEY;

  try {
    const raw = await fetchBankJson(url, { apiKey });
    return {
      ...normalizeBankPayload(raw, fallback),
      source: "sbi_api",
      live: true,
    };
  } catch (error) {
    console.warn("[BANK:SBI] external API failed, using fallback:", error.message);
    return { ...fallback, source: "fallback", live: false, error: error.message };
  }
}

module.exports = { fetchSbiRates };
